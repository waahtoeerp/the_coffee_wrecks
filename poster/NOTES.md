...
**Why the asymmetry, and why it's not a plotting/data issue:** confirmed directly from the raw `3pGtoA_freq.txt` values (not just the plot) — e.g. R0002 position-1 G→A is 0.0078 vs. C→T's 0.0316 (~4x lower), and it stays flat/noisy across all 25 positions rather than showing a clear decaying peak like C→T does. Same pattern in all 4 samples. In a *double-stranded* aDNA library, you'd expect near-mirror-image curves (5′ C→T and 3′ G→A both spiking and decaying inward, since both strands carry terminal deamination and get sequenced from both directions — this is the classic symmetric picture, e.g. Briggs et al. 2007). A strong 5′ C→T with a much weaker/flat 3′ G→A is the known signature of *single-stranded library preparation* (or a double-stranded prep with an end-repair/fill-in step), where the overhang chemistry isn't symmetric the way double-stranded ligation is — this is the most likely explanation here. Partial UDG/USER treatment could also flatten the signal, but would be expected to truncate *both* ends, which isn't what we see (C→T remains strong), so it's a less likely explanation. **Open item:** the library prep kit/protocol isn't documented anywhere in this pipeline's records — worth confirming with the sequencing facility before finalizing poster wording, so the asymmetry can be stated as "consistent with [specific protocol]" rather than left unexplained.
