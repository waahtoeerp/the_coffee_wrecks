...
- `visualization/` scripts (`vcf_stats.sh`, `vcf_plot.sh`, `extract_snp_viz_data.sh`) still hardcode `SAMPLE=CT600-007R0002` — the base pipeline (rows 1, 3–9) is generalized (`SAMPLE` as `$1`, see below) but visualization isn't yet. Pending once all 4 samples' base analysis is verified.
